...
// Copyright (c) 2021 Georgios Konstantopoulos. MIT OR Apache-2.0.
// See src/vendor/mod.rs for why this is a copy rather than a dependency.
...
impl<'a, R: Read + Seek> BinFile<'a, R> {
    fn new(reader: &'a mut R) -> IoResult<Self> {
        ...
        Ok(Self {
            // DIVERGENCE: upstream unwraps here, so four non-UTF8 bytes at the
            // head of the file abort the process. Measured: flipping byte 0 to
            // 0xFF panicked.
            ftype: std::str::from_utf8(&magic[..])
                .map_err(|_| {
                    SerializationError::IoError(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        "zkey magic is not valid UTF-8",
                    ))
                })?
                .to_string(),
            version,
    ...
    // Upstream is `self.sections.get(&id).unwrap()[0].clone()`, which panics twice
    // over: once when the section is absent, once when its vector is empty. A
    // .zkey missing any of sections 2, 3, 4 or 5..9 aborts the process. Measured
    // with a hand-built file under 100 bytes: three of four hostile inputs
    // panicked rather than returning Err.
    //
    // `read_zkey` is `pub` and re-exported, so a consumer can hand it network
    // bytes even though this crate only ever passes it a local file.
    fn get_section(&self, id: u32) -> IoResult<Section> {
        self.sections
            ...
            .ok_or_else(|| {
                SerializationError::IoError(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("zkey is missing section {id}"),
                ))
            })
    }
    ...
    pub fn matrices(&mut self) -> IoResult<ConstraintMatrices<Fr>> {
        ...
        for _ in 0..num_coeffs {
            ...
            // DIVERGENCE: upstream indexes straight into `matrices` with two
            // unvalidated u32s from the file. `matrix` addresses a 2-element vec,
            // so any value >= 2 panics, and `constraint` addresses a
            // domain_size-length vec with no bound of its own.
            let row = matrices.get_mut(matrix as usize).ok_or_else(|| {
                SerializationError::IoError(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("zkey coefficient names matrix {matrix}, but only A and B exist"),
                ))
            })?;
            let cell = row.get_mut(constraint as usize).ok_or_else(|| {
                SerializationError::IoError(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!(
                        "zkey coefficient names constraint {constraint}, past the \
                         {} the header declares",
                        header.domain_size
                    ),
                ))
            })?;
            cell.push((value, signal as usize));
        }

        // DIVERGENCE: upstream is `max_constraint_index as usize - header.n_public`,
        // which underflows when a file declares more public inputs than it has
        // constraints — a panic in debug, a huge usize in release that then feeds
        // `truncate` and the returned matrices.
        let num_constraints = (max_constraint_index as usize)
            .checked_sub(header.n_public)
            .ok_or_else(|| {
                SerializationError::IoError(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!(
                        "zkey declares {} public inputs but its largest constraint index is {}",
                        header.n_public, max_constraint_index
                    ),
                ))
            })?;
        ...
        let matrices = ConstraintMatrices {
            ...
            // DIVERGENCE: upstream subtracts unchecked; n_public > n_vars underflows.
            num_witness_variables: header.n_vars.checked_sub(header.n_public).ok_or_else(|| {
                SerializationError::IoError(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!(
                        "zkey declares {} public inputs but only {} variables",
                        header.n_public, header.n_vars
                    ),
                ))
            })?,
            num_constraints,
...
// DIVERGENCE FROM UPSTREAM: both constructors below were `G1Affine::new` /
// `G2Affine::new`, which assert `is_on_curve()` **in release builds too** — an
// abort, not a panic, so `catch_unwind` cannot recover and wasm has no recourse
// at all. Measured by flipping one byte at 30 positions across a real .zkey: 27
// of them aborted with `assertion failed: point.is_on_curve()`.
//
// `new_unchecked` plus an explicit check is the same pattern `format/snarkjs.rs`
// already uses for the JSON path, and for the same reason.
fn on_curve_err(what: &str) -> SerializationError {
    SerializationError::IoError(std::io::Error::new(
        std::io::ErrorKind::InvalidData,
        format!("zkey contains a {what} point that is not on the curve"),
    ))
}
...
fn deserialize_g2<R: Read>(reader: &mut R) -> IoResult<G2Affine> {
    ...
    if infinity {
        ...
        let p = G2Affine::new_unchecked(f1, f2);
        if !p.is_on_curve() {
            return Err(on_curve_err("G2"));
        }
        Ok(p)
    }
}
